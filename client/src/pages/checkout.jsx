...
function Checkout() {
  ...
  return (
    <div className="min-h-screen bg-[#1E1B1B] text-[#F7F3F0] p-6 md:p-12">
      <div className="max-w-6xl mx-auto">
        
        {/* Page Title */}
        <h1 className="text-3xl font-bold mb-2 tracking-wide">Secure Checkout</h1>
        <p className="text-gray-400 mb-10">Complete your details to bring your handmade gifts home.</p>

        {/* TWO COLUMN GRID LAYOUT */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          
          {/* COLUMN 1 & 2: SHIPPING DETAILS FORM */}
          <div className="lg:col-span-2 bg-[#2A2525] rounded-[25px] p-6 md:p-8 border border-gray-800 shadow-xl">
            <h2 className="text-xl font-semibold text-[#D98C95] mb-6 border-b border-gray-800 pb-3">
            ...
            <form onSubmit={handlePlaceOrder} className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input 
                  ...
                  className="bg-[#1E1B1B] text-[#F7F3F0] px-4 py-3 rounded-xl border border-gray-800 focus:outline-none focus:border-[#D98C95] transition-colors text-sm"
                />
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <textarea 
                  ...
                  className="bg-[#1E1B1B] text-[#F7F3F0] px-4 py-3 rounded-xl border border-gray-800 focus:outline-none focus:border-[#D98C95] transition-colors text-sm resize-none"
                />
              </div>

              {/* PAYMENT SELECTION METHOD */}
              <h2 className="text-xl font-semibold text-[#D98C95] mt-4 mb-2 border-b border-gray-800 pb-3">
                💳 Payment Method
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div>
                    <p className="font-semibold text-sm">Instant UPI Payment</p>
                    <p className="text-xs text-gray-400 mt-0.5">Pay via GPay, PhonePe, or Paytm</p>
                  </div>
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${paymentMethod === 'upi' ? 'border-[#D98C95]' : 'border-gray-600'}`}>
                ...
                >
                  <div>
                    <p className="font-semibold text-sm">Cash / UPI on Delivery</p>
                    <p className="text-xs text-gray-400 mt-0.5">Pay at your doorstep when it arrives</p>
                  </div>
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${paymentMethod === 'cod' ? 'border-[#D98C95]' : 'border-gray-600'}`}>
          ...
          <div className="bg-[#2A2525] rounded-[25px] p-6 border border-gray-800 shadow-xl flex flex-col gap-6 lg:sticky lg:top-6">
            ...
            <div className="flex flex-col gap-4 max-h-60 overflow-y-auto pr-1">
              {DUMMY_CHECKOUT_ITEMS.map((item) => (
                <div key={item.id} className="flex gap-3 items-center bg-[#1E1B1B] p-3 rounded-xl border border-gray-800/40">
                  ...
                  <p className="text-xs font-semibold text-[#D98C95]">₹{item.price * item.qty}</p>
                </div>
              ))}
            </div>

            {/* Cost Breakdown */}
            <div className="flex flex-col gap-3 text-sm border-t border-b border-gray-800 py-4 my-2">
            ...
            <button 
              ...
              className="w-full bg-[#D98C95] text-black font-bold py-3 rounded-full hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 text-center shadow-lg text-sm"
            >
